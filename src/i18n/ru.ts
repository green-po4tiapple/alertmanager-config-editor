...
export const ru = {
  ...
  load: {
    ...
    subtitle:
      'Вставьте `alertmanager_config.yaml` целиком либо только блок `route:` (можно и просто ' +
      'список веток со `- matchers:`) — или HelmRelease, внутри которого он лежит. Разбор идёт ' +
      'в браузере, конфиг наружу не уходит и в браузерном хранилище не сохраняется.',
    ...
    username: 'логин',
    password: 'пароль',
    orPaste: 'или вставьте вручную',
    loadTree: 'Загрузить дерево',
    openFile: 'Открыть файл…',
    loadExample: 'Пример',
    exampleTitle: 'Загрузить демонстрационный конфиг — потрогать редактор без своих данных',
    dropHint: 'или перетащите файл в поле · ⌘/Ctrl+Enter',
    factFeatures:
      '**Что редактор умеет.** Два режима отображения одного дерева (блочный список и ' +
      'диаграмма), перемещение и переподвешивание веток, проверка произвольного набора ' +
      'лейблов с показом всех receiver’ов (с учётом `continue`), экспорт готового ' +
      '`route:`-блока.',
    factSecrets:
